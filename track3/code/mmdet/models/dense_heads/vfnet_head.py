...
class VFNetHead(ATSSHead, FCOSHead):
    ...
    def _init_layers(self):
        ...
        self.vfnet_cls_dconv = DeformConv2d(
            ...
            padding=self.dcn_pad)
        # nonlocal
        self.vfnet_cls_nonlocal = NonLocalBlockND(in_channels=self.feat_channels)
        '''
        # ocr
        #ocr_key_channels = 256
        ocr_key_channels = self.feat_channels
        ...
        self.ocr_gather_head = SpatialGather_Module(self.cls_out_channels)
        self.ocr_distri_head = SpatialOCR_Module(in_channels=self.feat_channels,
    ...
    def forward_single(self, x, scale, scale_refine, stride, reg_denom):
        ...
        # nonlocal
        cls_feat = self.vfnet_cls_nonlocal(cls_feat)
        '''
        # ocr
        out_aux = self.aux_head(cls_feat)
        cls_feat = self.conv3x3_ocr(cls_feat)
        context = self.ocr_gather_head(cls_feat, out_aux)
        cls_feat = self.ocr_distri_head(cls_feat, context)
        '''
        # vfnet cls head  
        cls_score = self.vfnet_cls(cls_feat)

        if self.training:
            return cls_score, bbox_pred, bbox_pred_refine, cls_feat
            #return cls_score + out_aux, bbox_pred, bbox_pred_refine
        else:
    ...
    @force_fp32(apply_to=('cls_scores', 'bbox_preds', 'bbox_preds_refine', 'cls_feat'))
    def loss(self,
             cls_scores,
             bbox_preds,
             bbox_preds_refine,
